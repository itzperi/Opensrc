...
describes.sandboxed('3p', {}, (env) => {
  ...
  it('should do work only in primary frame', () => {
    ...
    primaryFrame.context.primaryFrame = primaryFrame;
    const workerFrame0 = {
      context: {
    ...
    computeInPrimaryFrame(workerFrame0, taskId, work, frame('workerFrame0'));
    expect(workCalls).to.equal(0);
    computeInPrimaryFrame(primaryFrame, taskId, work, frame('primaryFrame'));
    expect(workCalls).to.equal(1);
    computeInPrimaryFrame(workerFrame1, taskId, work, frame('workerFrame1'));
    expect(progress).to.equal('');
    done(';');
    expect(progress).to.equal(';workerFrame0;primaryFrame;workerFrame1');
    computeInPrimaryFrame(workerFrame2, taskId, work, frame('workerFrame2'));
    expect(progress).to.equal(';workerFrame0;primaryFrame;workerFrame1;workerFrame2');
    expect(workCalls).to.equal(1);
  });
