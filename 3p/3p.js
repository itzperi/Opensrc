...
export function computeInPrimaryFrame(global, taskId, work, cb) {
  const {primaryFrame} = global.context;
  let tasks = primaryFrame.__ampPrimaryFrameTasks;
  if (!tasks) {
    tasks = primaryFrame.__ampPrimaryFrameTasks = {};
  }
  ...
  cbs.push(cb);
  if (!global.context.isPrimaryFrame) {
